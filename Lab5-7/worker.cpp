...
int main(int argc, char **argv) {
    ...
    while (1) {
        if (getppid() == 1) {  // Если родительский процесс мертв, завершаем узел
            std::cout << "[WORKER] Parent process died. Exiting..." << std::endl;
            break;
        }
        std::string message;
        std::string command = " ";
        ...
        if (command == "create") {
            ...
            if (cmd == "start") {
                ...
                if (timer_running) {
                    ...
                    auto diff = elapsed + std::chrono::duration<double>(now - start_time);
                    ans = "Ok:" + std::to_string(me.id) + ":elapsed " + std::to_string(static_cast<int>(diff.count() * 1000)) + " ms";
                } else {
                    ans = "Ok:" + std::to_string(me.id) + ":elapsed " + std::to_string(static_cast<int>(elapsed.count() * 1000)) + " ms";
                }
            }else if (cmd == "stop") {
                if (timer_running) {
                    auto now = std::chrono::steady_clock::now();
                    elapsed += std::chrono::duration<double>(now - start_time); // Сохраняем время
                    timer_running = false;
                    ans = "Ok:" + std::to_string(me.id) + ":timer stopped at " + std::to_string(static_cast<int>(elapsed.count() * 1000)) + " ms";
                } else {
